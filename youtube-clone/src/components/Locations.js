...
const Locations = ({ setShowLocations,setshowMoreSettings }) => {
  ...
  return (
    ...
    <div
      className={ `AccountContainer md:hidden overflow-y-auto  bg-white  w-full h-screen fixed  top-0 right-0 bottom-0 left-0  z-40`}
    >
      <div className="px-3 h-14 bg-white sticky top-0 right-0 left-0 shadow-[0px_4px_2px_-2px_#00000033] flex justify-between items-center">
        <div
        ...
        <div className="text-lg  w-11/12">Location</div>
      </div>
      <div className="LocationsList">
        {countries.map((country)=>{
            return <Country name={country.name} code={country.code} setShowLocations={setShowLocations}/>
        })}
      </div>
    </div>
    ...
    {/* for large device */}
    <div
      className={ `locationsContainer hidden md:block  AccountContainer overflow-y-auto  bg-white  w-full h-[100vh] fixed  -top-16 right-0 bottom-0 left-0 rounded-xl backdrop-blur  z-40`}
    >
      <div className="px-3 h-14 bg-white sticky top-0 right-0 left-0  flex justify-between items-center">
        <div
